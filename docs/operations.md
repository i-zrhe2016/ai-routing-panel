...
- Compose 还会在打包前通过宿主机 Tailscale SSH 以只读方式采集普通数据面和已配置远端 AI 节点；没有远端 AI 目标时，本机 AI 备用的 `.env` 与 `config-ai-node.json` 随 `config/` 归档
- 每个归档都包含 `node-recovery-manifest.json`，任务还会生成 `node-recovery-status.json`，明确标记 `recoveryReady`
- 远端采集默认是必需的：配置节点无法通过 Tailscale SSH 提供恢复文件时，本次归档不会继续上传；计划维护时才设置 `DB_BACKUP_SSH_COLLECTION_REQUIRED=0` 和 `DB_BACKUP_RECOVERY_REQUIRED=0`
